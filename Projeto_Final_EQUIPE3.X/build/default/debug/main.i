...
# 52 "C:\Program Files\Microchip\xc8\v2.45\pic\include\proc\pic16f1827.h"
...
# 76 "C:\Program Files\Microchip\xc8\v2.45\pic\include\pic.h"
...
# 118 "C:\Program Files\Microchip\xc8\v2.45\pic\include\pic.h"
...
for (int i = 0; i < 5; i++) {
if (filaSolicitacoes[i].origem == andar) {
filaSolicitacoes[i].origem = 8;
if (checarDestino && filaSolicitacoes[i].destino == andar) {
...
flagDestinos[andar] = 0;
for (int i = 0; i < 5; i++) {
if (filaSolicitacoes[i].destino == andar) {
flagDestinos[andar] = 1;
break;
}
}
}
...
bool temDestino = flagDestinos[andar] == 1;
bool temOrigem = flagOrigens[andar] == 1;
if (temOrigem) {
PararElevadorEAtualizarEstado(andar);
AtualizarFilaSolicitacoes(andar, temDestino);
}

if (temDestino && !temOrigem) {
PararElevadorEAtualizarEstado(andar);
}
AtualizarFlagDestino(andar);
}
...
uint8_t data[2];
tempo_captura = ((float)valor_capturado)*0.000001;
velocidade = (uint16_t)(0.837/tempo_captura);
data[0] = (uint8_t)(velocidade >> 8);
data[1] = (uint8_t)(velocidade & 0xFF);
txSpi(data, 2);
