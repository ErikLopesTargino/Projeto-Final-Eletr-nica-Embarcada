...
# 345 "./mcc_generated_files/pin_manager.h"
extern void (*IOCBF0_InterruptHandler)(void);
# 369 "./mcc_generated_files/pin_manager.h"
...
# 453 "./mcc_generated_files/pin_manager.h"
...
void CMP1_ISR(void);
# 161 "./mcc_generated_files/cmp1.h"
void CMP1_SetInterruptHandler(void (* InterruptHandler)(void));
# 172 "./mcc_generated_files/cmp1.h"
extern void (*CMP1_InterruptHandler)(void);
# 184 "./mcc_generated_files/cmp1.h"
void CMP1_DefaultInterruptHandler(void);
...
uint8_t countTMR0 = 0;
solicitacao filaSolicitacoes[5];
uint8_t matriz[8];
uint8_t totalSolicitacoes = 0;
uint8_t andar_atual = 0;
uint8_t flagDestinos[4] = {0, 0, 0, 0};
uint8_t flagOrigens[4] = {0, 0, 0, 0};
uint8_t enviar_TX[10] = {0,0,0,0,0,'.',0,0,'.',0};



enum movimento {descendo, subindo, parado};
enum movimento estado = parado;
enum movimento estado_anterior = parado;
...
uint8_t ascii2bin(uint8_t caractereAscii){
    if (caractereAscii < 0x3A) {
        return caractereAscii - 0x30;
    } else {
        return caractereAscii - 0x37;
    }
}
...
uint8_t bin2ascii(uint8_t valorBinario){
    valorBinario &= 0x0F;
    if (valorBinario < 10) {
        return valorBinario + 0x30;
    } else {
        return valorBinario + 0x37;
    }
}
...
void adicionaSolicitacaoNaFila(uint8_t pisoOrigem, uint8_t pisoDestino) {
    ...
    if (pisoOrigem != pisoDestino && totalSolicitacoes <= 5) {
        filaSolicitacoes[totalSolicitacoes].origem = pisoOrigem;
        filaSolicitacoes[totalSolicitacoes].destino = pisoDestino;
        flagOrigens[pisoOrigem] = 1;
        flagDestinos[pisoDestino] = 1;
        totalSolicitacoes++;
    }
}
...
{
    int i;
    if (flagDestinos[x] == 1 && flagOrigens[x] == 1){
        estado = parado;
        controlaMotorElevador(estado);
        _delay((unsigned long)((2000)*(16000000/4000.0)));
        flagOrigens[x] = 0;
        for (i=0;i<5;i++){
            if (filaSolicitacoes[i].origem == x)
                filaSolicitacoes[i].origem = 8;
            if (filaSolicitacoes[i].destino == x && filaSolicitacoes[i].origem == 8){
                filaSolicitacoes[i].destino = 8;
                totalSolicitacoes--;
            }
        }
    }
    else if (flagOrigens[x] == 1){
        estado = parado;
        controlaMotorElevador(estado);
        _delay((unsigned long)((2000)*(16000000/4000.0)));
        flagOrigens[x] = 0;
    ...
    for (i=0;i<5;i++){
        if (filaSolicitacoes[i].destino == x){
            flagDestinos[x] = 1;
            break;
        }
        else
...
void Andar0() {
    andar_atual = 0;
    CompletaSolicitacao(andar_atual);
...
void Interrupt_CCP4(uint16_t valor_capturado) {
    tempo_captura = ((float)valor_capturado)*0.0000001;
    if (estado = subindo)
        posicao++;
    else
        posicao--;
...
void trasmiteDadosUart(){
    float velocidade, temperatura, altura;
    uint16_t velocidade_inteiro, velocidade_decimal,temperatura_inteiro, temperatura_decimal;
    uint8_t destino, estado_num;
    int j = 0;
    ...
    if (countTMR0 == 20){
        ...
        if(estado==subindo){
            estado_num=(uint8_t)3;
            if (andar_atual != 3)
                destino = andar_atual+1;
            else
                destino = 2;
        } if (estado==descendo) {
            estado_num=(uint8_t)2;
            if (andar_atual != 0)
                destino = andar_atual-1;
            else
                destino = 0;
        } else {
            estado_num=(uint8_t)0;
            destino = andar_atual;
        }

        velocidade = (180/215)/tempo_captura;
        velocidade_inteiro = (uint16_t)(velocidade);
        velocidade_decimal = ((uint16_t)(velocidade))-velocidade_inteiro*10;
        temperatura = (float)ADC_GetConversion(Temp)*0.225;
        temperatura_inteiro = (uint16_t)(temperatura);
        temperatura_decimal = ((uint16_t)(temperatura))-temperatura_inteiro*10;
        altura = posicao*(180/215);

        enviar_TX[0] = andar_atual;
        enviar_TX[1] = destino;
        enviar_TX[2] = estado;
        enviar_TX[3] = (uint8_t)altura;
        enviar_TX[4] = (uint8_t)(velocidade_inteiro);
        enviar_TX[6] = (uint8_t)(velocidade_decimal);
        enviar_TX[7] = (uint8_t)(temperatura_inteiro);
        enviar_TX[9] = (uint8_t)(temperatura_decimal);

        EUSART_Write('$');

        for(j=0;j<5;j++){
            EUSART_Write(bin2ascii(enviar_TX[j]));
        }
        EUSART_Write(enviar_TX[5]);
        for(j=0;j<2;j++){
            EUSART_Write(bin2ascii(enviar_TX[j+6]));
        }
        EUSART_Write(enviar_TX[8]);
        EUSART_Write(bin2ascii(enviar_TX[9]));
        EUSART_Write(0x0D);
...
void main(void)
{
    ...
    IOCBF0_SetInterruptHandler(Andar0);
    IOCBF3_SetInterruptHandler(Andar1);
    CMP1_SetInterruptHandler(Andar2);
    CMP2_SetInterruptHandler(Andar3);
    ...
    while (1)
    {
        if(EUSART_is_rx_ready()){
            ...
            else if (rx_char == 0x0D && cont_RX == 2){
                adicionaSolicitacaoNaFila(ascii2bin(buffer[0]),ascii2bin(buffer[1]));
                Andar0();
            }
            else
                if (cont_RX < 2){
                    buffer[cont_RX] = rx_char;
                    cont_RX++;
                }
        }
    }
